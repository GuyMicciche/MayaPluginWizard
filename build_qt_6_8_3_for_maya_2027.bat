...
REM Step 2: Switch to 6.8.3 tag
echo Step 2: Switch to 6.8.3 tag
cd /d "%QTSRC%"
git checkout 6.8.3
if errorlevel 1 (echo ERROR: failed to checkout 6.8.3 & pause & exit /b 1)
...
REM Step 4: Wipe build dir, recreate with qtbase subdir
echo Step 4: Prepare build dir
if exist "%QTBUILD%" rmdir /s /q "%QTBUILD%"
mkdir "%QTBUILD%\qtbase"
cd /d "%QTBUILD%\qtbase"

REM Step 5: Call qtbase's configure with -top-level
REM Configures all submodules; build artifacts land in %QTBUILD%
echo Step 5: Configure
call "%QTSRC%\qtbase\configure.bat" -top-level -prefix "%QTINSTALL%" -opensource -confirm-license -release -nomake examples -nomake tests
if errorlevel 1 (echo ERROR: configure failed & pause & exit /b 1)

REM Step 6: Build (from the top-level build dir)
echo Step 6: Build
cd /d "%QTBUILD%"
